...
標示 `【待主網】` 的欄位需在 BNB Chain 主網部署完成後填入。
標示 `【待法務】` 的欄位需律師與託管方確認後填入,**不得填寫無法佐證的內容**。
...
定價管線(Keeper)每 10 分鐘執行一輪:5 個來源各推一筆報價 + 1 筆驗證 = 每輪 6 筆,
**約每日 860 筆**。這些是產品在運行,不是測試調用。

#### (b) 多個錢包地址交互

| 類別 | 說明 |
| --- | --- |
| 基礎設施錢包 | 4 個獨立報價者,各自對應一個真實資料來源 |
| 一般使用者錢包 | 提案、啟用、觸發驗證 —— 皆為無需許可操作 |

#### (c) 可驗證的真實用戶數量

【待主網】— 封閉 Beta 參與者地址清單另附。

> **誠實聲明:** 測試網階段的參與錢包均為專案自行操作,**不計入真實用戶數**。
> 主網 Beta 的參與者為獨立第三方,其地址具有各自的鏈上歷史,可逐一查核。
...
逐筆交易雜湊與 BscScan 連結見 [`onchain-flow.md`](./onchain-flow.md),
...
| 3 | GitHub 連結 | 【待補】 |
| 4 | 產品連結 | 【待主網】 |
| 5 | 鏈上交互數據網址 | [`onchain-activity.md`](./onchain-activity.md) + BscScan + 【Dune】 |
...
| 不合格情形 | 本專案狀況 |
| --- | --- |
| (1) 無真實鏈上交互 | 定價管線持續運行,每日約 1,440 筆真實業務交易 |
